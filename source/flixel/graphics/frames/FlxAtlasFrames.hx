...
class FlxAtlasFrames extends FlxFramesCollection
{
    ...
    static function adobeHelper(FrameName:String, FrameData:Dynamic, Frames:FlxAtlasFrames):Void
    {
		var rotated:Bool = FrameData.rotated;
		var name:String = FrameName;
		var sourceSize:FlxPoint = FlxPoint.get(FrameData.sourceSize.w, FrameData.sourceSize.h);
    ...
    public static function fromJson(Source:FlxGraphicAsset, Description:FlxTexturePackerSource):FlxAtlasFrames
    {
		var graphic:FlxGraphic = FlxG.bitmap.add(Source, false);
		if (graphic == null)
			return null;
