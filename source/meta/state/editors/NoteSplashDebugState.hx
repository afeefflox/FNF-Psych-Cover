...
class NoteSplashDebugState extends MusicBeatState
{
	...
	override function create()
	{
		...
		imageInputText = new FlxInputText(txtx, txty - 30, 360, 'noteSplashes/noteSplashes', 16);
		blockPressWhileTypingOn.push(imageInputText);
		add(imageInputText);

		var reloadImage:FlxButton = new FlxButton(imageInputText.x + 510, imageInputText.y - 3, "Reload Image", function()
		{
		...
		blockPressWhileTypingOnStepper.push(stepperMinFps);
		stepperMinFps.name = 'min_fps';
		add(stepperMinFps);

		stepperMaxFps = new FlxUINumericStepper(txtx + 60, txty - 60, 1, 26, 1, 60, 0);
		blockPressWhileTypingOnStepper.push(stepperMaxFps);
	...
	override function update(elapsed:Float)
	{
		...
		if(!blockInput) {
			for (stepper in blockPressWhileTypingOnStepper) {
				...
				var leText:FlxInputText = leText;
				if(leText.hasFocus) {
					ClientPrefs.toggleVolumeKeys(false);
					blockInput = true;
					break;
	...
	{
		if(image == 'noteSplashes' || image == 'noteSplashes/noteSplashes')
			texturePath = NoteSplash.defaultNoteSplash + NoteSplash.getSplashSkinPostfix();
		else
			texturePath = image;
