		...
		else if(controls.RESET && hexTypeNum < 0)
		{
			if(FlxG.keys.pressed.SHIFT)
			{
				for (i in 0...3)
				{
					...
					var color:FlxColor = !onPixel ? ClientPrefs.arrowRGB[curSelectedNote][i] :
													ClientPrefs.arrowRGBPixel[curSelectedNote][i];
					switch(i)
					{
						case 0:
			...
			setShaderColor(!onPixel ? ClientPrefs.arrowRGB[curSelectedNote][curSelectedMode] : ClientPrefs.arrowRGBPixel[curSelectedNote][curSelectedMode]);
			FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
			updateColors();
		}
