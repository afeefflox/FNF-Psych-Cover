...
import objects.FlxSymbol.Parsed;
import flixel.FlxG;
...
class FlxAnimate extends FlxSymbol
{
	...
	public function new(x:Float, y:Float, key:String, ?library:String)
	{
		var folder:String = 'tightBars';
		coolParse = cast Json.parse(Paths.getTextFromFile('images/$key/Animation.json', library));
		coolParse.AN.TL.L.reverse();
		super(x, y, coolParse);

		frames = FlxAnimate.fromAnimate(Paths.image('$key/spritemap1', library), Paths.getTextFromFile('images/$key/spritemap1.json', library));
		// frames
	}
