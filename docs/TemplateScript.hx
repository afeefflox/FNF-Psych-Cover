...
function opponentNoteHit(note:Note, character:Array<Character>, chararterStrums:StrumLineNote)
{
...
function fakeOpponentNoteHit(note:Note, character:Array<Character>, chararterStrums:StrumLineNote)
{
    // it nothing tho just work for fake character as double chart thing 
}

function noteMissPress(direction:Int, character:Array<Character>)
{
	// Called after the note press miss calculations
	// Player pressed a button, but there was no note to hit (ghost miss)
}

function noteMiss(note:Note, character:Array<Character>)
{
	// Called after the note miss calculations
	// Player missed a note by letting it go offscreen
}

function noteHit(note:Note, character:Array<Character>, chararterStrums:StrumLineNote)
{
...
function onMoveCamera(focus:String)
{
    ...
    //better way you can do is this lmao
    switch(focus)
    {
        case 'boyfriend':
        case 'dad':
    }
    // I LOVE USING SWITCH STATEMATE STRING YK
}
...
//Icon Function (only work in images/icons)
function changeIcon(char:String, newPlayer:Bool)
{
    // char is icon name 
    // newPlayer is nothing cool about it just when you enabled flip icon
}

function updateHitbox() {
    // updateHitbox on icons
}

function updateAnims(health:Float) {
    // called when health is bigger or small (NOTE: the code has to be player health not opponent health)
}
